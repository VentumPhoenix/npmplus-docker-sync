...
func (w *Worker) flush() error {
	ctx, cancel := context.WithTimeout(context.WithoutCancel(context.Background()), w.opts.ShutdownTimeout)
	defer cancel()

	w.log.Info("flushing pending state before shutdown")
...
func (w *Worker) Reconcile(ctx context.Context) (Result, error) {
	...
	for _, kind := range w.opts.Kinds {
		kindResult, err := w.reconcileKind(ctx, kind, byKind[kind])
		res.add(kindResult)
		if err != nil {
...
func (w *Worker) reconcileKind(ctx context.Context, kind npm.Kind, targets []*docker.Target) (Result, error) {
	var res Result
	...
	for _, key := range sortedKeys(existing) {
		...
		if !w.opts.DeleteOrphans {
			res.Skipped++
			log.Debug("orphaned resource kept (DELETE_ORPHANS=false)",
